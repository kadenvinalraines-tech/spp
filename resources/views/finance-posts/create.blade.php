...
@section('title', 'Tambah Pos Keuangan')

@section('content')
<div class="card shadow-sm">
    <div class="card-header bg-white">
        <h5 class="mb-0">Tambah Pos Keuangan</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('finance-posts.store') }}" method="POST">
            @csrf
            
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="name" class="form-label">Nama Pos Keuangan <span class="text-danger">*</span></label>
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: SPP Bulanan Kelas X" required>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select mb-2" id="type_select" onchange="checkCustomType()">
                        ...
                        <option value="SPP" {{ old('type') == 'SPP' ? 'selected' : '' }}>SPP</option>
                        <option value="Gedung" {{ old('type') == 'Gedung' ? 'selected' : '' }}>Gedung</option>
                        <option value="Seragam" {{ old('type') == 'Seragam' ? 'selected' : '' }}>Seragam</option>
                        <option value="Prakerin" {{ old('type') == 'Prakerin' ? 'selected' : '' }}>Prakerin</option>
                    ...
                    <input type="hidden" name="type" id="real_type" value="{{ old('type') }}">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control @error('default_amount') is-invalid @enderror" id="default_amount" name="default_amount" value="{{ old('default_amount', 0) }}" min="0" required>
            ...
            <div class="d-flex justify-content-end">
                ...
                <button type="submit" class="btn btn-primary">Simpan Pos Keuangan</button>
            </div>
        </form>
    </div>
</div>
